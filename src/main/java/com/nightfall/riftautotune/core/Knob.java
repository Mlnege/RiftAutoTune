 ...
 *   <li>{@code baseCostMs} &mdash; approximate frame-time cost (ms) per level step on a MEDIUM
 *       machine, before per-machine calibration from the benchmark sweep.</li>
...
public enum Knob {
    ...
    // ---- Vanilla ----------------------------------------------------------------
    RENDER_DISTANCE      ("Render Distance", new int[]{4, 6, 8, 10, 12, 16, 24, 32}, 1.40, 0.45, false, false),
    SIMULATION_DISTANCE  ("Simulation Distance", new int[]{4, 6, 8, 10, 12},          0.45, 0.50, false, false),
    ENTITY_DISTANCE      ("Entity Distance %", new int[]{50, 75, 100, 150, 200},      0.45, 0.30, false, false),
    GRAPHICS_MODE        ("Graphics", new int[]{0, 1, 2},                              1.00, 0.80, false, false),
    BIOME_BLEND          ("Biome Blend", new int[]{0, 1, 2, 3},                        0.20, 0.25, false, false),
    CLOUDS               ("Clouds", new int[]{0, 1, 2},                                0.30, 0.25, false, false),
    PARTICLES            ("Particles", new int[]{0, 1, 2},                             0.30, 0.20, false, false),

    // ---- Shaders (Oculus) -------------------------------------------------------
    // Master toggle. Gated by HardwareProfile/shader availability in CostModel, not here.
    SHADERS              ("Shaders", new int[]{0, 1},                                  4.00, 6.00, false, false),
    SHADER_SHADOW_RES    ("Shadow Map Res", new int[]{512, 1024, 2048, 4096},          1.20, 1.60, true,  false),
    SHADER_SHADOW_DIST   ("Shadow Distance", new int[]{0, 1, 2, 3},                    0.60, 0.80, true,  false),
    SHADER_VOLUMETRIC    ("Volumetric Fog", new int[]{0, 1, 2, 3},                     0.90, 1.10, true,  false),
    SHADER_SSAO          ("SSAO", new int[]{0, 1, 2},                                  0.50, 0.60, true,  false),
    SHADER_BLOOM         ("Bloom", new int[]{0, 1},                                    0.40, 0.30, true,  false),

    // ---- Distant Horizons -------------------------------------------------------
    // Master toggle (level 0 == DH off). Gated by DH availability in CostModel.
    DH_LOD_DISTANCE      ("DH LOD Distance", new int[]{0, 64, 128, 256, 512},          2.00, 1.00, false, true),
    DH_CPU_LOAD          ("DH CPU Load", new int[]{0, 1, 2},                           0.20, 1.30, false, true),
    DH_VERTICAL_QUALITY  ("DH Vertical Quality", new int[]{0, 1, 2},                   0.50, 0.60, false, true),
    DH_LOD_DETAIL        ("DH LOD Detail", new int[]{0, 1, 2},                         0.60, 0.70, false, true);
    ...
    Knob(String displayName, int[] values, double visualWeight, double baseCostMs,
         boolean requiresShaders, boolean requiresDistantHorizons) {
